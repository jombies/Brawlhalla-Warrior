...
public class ChestLootPopper : MonoBehaviour, IPoolable
{
    ...
    void OpenChest()
    {
        ...
        if (TryGetComponent<PooledObject>(out PooledObject pooledObject)) {
            pooledObject.SetAutoReturnTime(8);
        }
    ...
    {
        chestLid.transform.localRotation = Quaternion.Euler(0, 0, 0); // Reset lid rotation
        isOpen = false; // Reset trạng thái mở
    }
}
[System.Serializable]
